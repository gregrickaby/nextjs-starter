  ...
  return (
    <Layout title={post.title.rendered} description={post.excerpt.rendered}>
      ...
      >
        ...
        <p className="pt-4">
          Static Generation (SSG) example via{' '}
...
export async function getStaticPaths() {
  const posts = await fetcher('https://gregrickaby.com/wp-json/wp/v2/posts')
...
export async function getStaticProps({params}) {
  const post = await fetcher(
    `https://gregrickaby.com/wp-json/wp/v2/posts/${params.id}`
  )
